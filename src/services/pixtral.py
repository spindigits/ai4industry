...
class PixtralPDFProcessor:
    ...
    def convert_pdf_to_images(
        ...
        """
        Convertit chaque page PDF en image PIL via PyMuPDF (fitz).
        ...
        try:
            ...
            for page in doc:
                # Calculer le zoom factor basé sur le DPI (72 est le défaut PDF)
                zoom = dpi / 72
                ...
                # PyMuPDF pixmap est en RGB par défaut (ou RGBA si alpha)
            ...
            print(f"Erreur conversion PDF (PyMuPDF): {e}")
            raise e
...
Réponds UNIQUEMENT en JSON valide, sans markdown."""
                ...
                try:
                    ...
                    if "429" in error_msg or "Rate limit" in error_msg:
                        if attempt < max_retries:
                            print(f"⚠️ Rate limit atteint (Page {page_num}), nouvelle tentative dans {retry_delay}s... ({attempt+1}/{max_retries})")
                            time.sleep(retry_delay)
                            retry_delay *= 2  # Exponential backoff
                            continue
                    raise api_err # Relancer si ce n'est pas un rate limit ou si max retries atteint
    ...
    def create_enriched_chunks(
        ...
        Args:
            page_analyses: Résultats d'analyse
            source_path: Chemin du fichier source (PDF ou Image)
        ...
        for page_data in page_analyses:
            ...
            # Déterminer le type de fichier pour les métadonnées
            source_type = "image" if source_path.lower().endswith(('.png', '.jpg', '.jpeg')) else "pdf"
            ...
            # Enrichir avec le nom du fichier pour la recherche par nom
            if main_text:
                main_text = f"Source File: {filename}\n\n{main_text}"
            ...
            if main_text.strip():
                doc = Document(
                    ...
                    metadata={
                        ...
                        "layout": analysis.get("document_structure", {}).get("document_structure", {}).get("layout_type", "unknown"),
            ...
            for idx, table in enumerate(analysis.get("tables", [])):
                ...
                table_text = f"""Source File: {filename}
...
Summary: {summary}"""
            ...
            for idx, visual in enumerate(analysis.get("visual_elements", [])):
                ...
                visual_text = f"""Source File: {filename}
VISUAL ELEMENT {idx + 1} (Page {page_num}):
Type: {visual_type}
...
Description: {visual_description}"""
    ...
    def process_pdf_complete(
        ...
        for idx, image in enumerate(images):
            ...
            cache_file = self.cache_dir / f"{Path(pdf_path).stem}_page_{idx}.json"
            if cache_file.exists():
                try:
                    ...
                    print(f"Loaded page {idx} from cache.")
                except Exception as e:
                ...
                if analysis["success"] and analysis.get("analysis"):
                    try:
                        ...
                        with open(cache_file, 'w', encoding='utf-8') as f:
                            json.dump(analysis["analysis"], f, ensure_ascii=False, indent=2)
                    except Exception as e:
                        print(f"Cache write error: {e}")
                
                # Check if analysis failed due to rate limit despite retries
                if not analysis["success"] and "Rate limit exceeded" in str(analysis.get("error", "")):
                     print(f"Skipping page {idx} due to persistent Rate Limit.")
                     # Fallback to dummy to keep going
                     analysis = {
                        "page_number": idx,
                        "analysis": {
                            "text_content": "[Skipped due to Rate Limit]",
                            "tables": [],
                            "visual_elements": [],
                            "document_structure": {}
                        },
                        "success": True, # Make it success so we generate a chunk saying "Skipped"
                        "error": "Rate Limit Skipped"
                     }

                # Pause pour respecter les rate limits (seulement si appel API)
                time.sleep(5)

            page_analyses.append(analysis)

            # Cache optionnel des images (seulement si demandé)
            if self.cache_images:
    ...
    def process_image_complete(
        ...
        """
        Pipeline pour traiter une image unique.
        """
        try:
            ...
            # Analyse
            analysis = self.analyze_page_with_pixtral(image, 1) # Page 1 par défaut
            
            # Gestion erreur rate limit identique à PDF
            if not analysis["success"] and "Rate limit exceeded" in str(analysis.get("error", "")):
                 analysis = {
                    "page_number": 1,
                    "analysis": {
            ...
            # Création chunks
            documents = self.create_enriched_chunks(
                [analysis], 
                image_path
            )
            ...
            print(f"Erreur process_image_complete: {e}")
            return []
