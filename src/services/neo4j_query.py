...
class Neo4jQuerier:
    ...
    def get_graph_context_for_question(self, question):
        ...
        # 1. Événements avec produits vendus aux salons (Complex Multi-hop)
        # Ex: "Quels événements ont utilisé des produits vendus à Pollutec Paris?"
        if any(word in question_lower for word in ["événements", "events", "déploiements"]):
            if any(word in question_lower for word in ["vendus", "sold", "salon", "tradeshow"]):
                location = None
        ...
        # 2. CO2 économisé par produit (Aggregation)
        # Ex: "Quel est le CO2 total économisé par le produit PG-M01?"
        ...
        # 3. Ventes aux collectivités (Filtering + Aggregation)
        # Ex: "Quels salons ont généré le plus de revenus avec les collectivités?"
        ...
        # 4. Projets R&D pour festivals (Deep Multi-hop)
        # Ex: "Quels projets R&D visent les produits utilisés aux festivals?"
        if any(word in question_lower for word in ["r&d", "recherche", "développement", "projets"]):
            # Relaxed condition: just "r&d" + "festival" or "event" context is enough often
            if any(word in question_lower for word in ["festival", "événements", "coûts", "réduire", "visent"]):
        ...
        # 5. Produits par type de batterie (Simple Filter)
        # Ex: "Quels produits avec batteries LiFePO4 ont été vendus?"
        ...
        # 6. Top Salons (Simple Aggregation)
        # Ex: "Quels sont les meilleurs salons?"
        if any(word in question_lower for word in ["top", "meilleurs", "plus", "salons", "revenus"]):
            # Avoid conflict with "dans quels salons..." query below
            if "dans quels salons" not in question_lower:
        ...
        # 7. Historique de ventes d'un produit (Nouveau) - MISSING previously
        # Ex: "Dans quels salons le PG-U01 a-t-il été vendu?"
        if any(word in question_lower for word in ["salons", "tradeshows", "vendu", "sold"]):
            product_ids = ["PG-U01", "PG-M01", "PG-P01", "PG-C01", "PG-M02"]
            for prod_id in product_ids:
                if prod_id.lower() in question_lower:
                    # Check if we also have "salon" context, if not already handled by #1
                    if "événements" not in question_lower: # Distinction from query #1
                        results = self.query_product_sales_across_tradeshows(prod_id)
                        if results:
                             context.append({
                                "query_type": f"sales_history_for_{prod_id}",
                                "results": results
                            })

        # 8. Événements alimentés par type de produit (Nouveau) - MISSING previously
        # Ex: "Quels événements ont été alimentés par du Solaire?"
        if any(word in question_lower for word in ["alimentés", "powered", "utilisant"]):
             category = None
             if "solaire" in question_lower or "solar" in question_lower:
                 category = "Solaire"
             elif "hybride" in question_lower or "hybrid" in question_lower:
                 category = "Hybride"
             
             if category:
                 results = self.query_events_powered_by_product_type(category)
                 if results:
                     context.append({
                        "query_type": f"events_powered_by_{category}",
                        "results": results
                    })
